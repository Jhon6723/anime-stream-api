...
describe('ModerationService', () => {
  ...
  beforeEach(() => {
    ...
    service = new ModerationService(prisma, events, { syncPendingSources: vi.fn() } as any);
  });
      ...
      prisma.episode.findMany.mockResolvedValue([
        {
          ...
          videoSources: [{ id: 'vs-1', provider: 'STREAMTAPE', status: 'READY' }],
        },
      ]);
  ...
  describe('auditLog', () => {
    it('returns logs with includes', async () => {
      prisma.moderationLog.findMany.mockResolvedValue([
        { id: 'log-1', action: 'APPROVE', moderator: { id: 'u-1', username: 'admin' } },
      ]);
  ...
  describe('approve', () => {
    it('updates episode to APPROVED and creates log', async () => {
      prisma.episode.findUnique.mockResolvedValue({ id: 'ep-1', animeId: 'anime-1', episodeNumber: 1 });
      prisma.episode.update.mockResolvedValue({ id: 'ep-1', moderationStatus: 'APPROVED' });
    ...
    it('throws NotFoundException when episode does not exist', async () => {
      ...
      await expect(service.approve('nonexistent', 'mod-1', 'reason')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('warn', () => {
    it('updates episode to WARNED and creates log', async () => {
      prisma.episode.findUnique.mockResolvedValue({ id: 'ep-1', animeId: 'anime-1', episodeNumber: 1 });
      prisma.episode.update.mockResolvedValue({ id: 'ep-1', moderationStatus: 'WARNED' });
      prisma.moderationLog.create.mockResolvedValue({ id: 'log-1' });

      const result = await service.warn('ep-1', 'mod-1', 'Wrong video', 'Check timestamp');
  ...
  describe('disable', () => {
    it('sets isEnabled to false and creates log', async () => {
      ...
      const result = await service.disable('ep-1', 'mod-1', 'Inappropriate content');
  ...
  describe('enable', () => {
    it('sets isEnabled to true and creates log', async () => {
      ...
      const result = await service.enable('ep-1', 'mod-1', 'Re-enabled after review');
  ...
  describe('findEpisode', () => {
    it('throws NotFoundException for non-existent episode', async () => {
      ...
      await expect(service.warn('nonexistent', 'mod-1', 'reason')).rejects.toThrow(NotFoundException);
      await expect(service.disable('nonexistent', 'mod-1', 'reason')).rejects.toThrow(NotFoundException);
      await expect(service.enable('nonexistent', 'mod-1', 'reason')).rejects.toThrow(NotFoundException);
    });
  });
});
