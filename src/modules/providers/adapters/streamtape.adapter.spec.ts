...
describe('StreamtapeAdapter', () => {
  ...
  beforeEach(() => {
    ...
    configService = {
      get: vi.fn((key: string) => {
        if (key === 'providers.streamtape.baseUrl')
          return 'https://api.streamtape.com';
  ...
  describe('deleteFile', () => {
    it('completes on success', async () => {
      ...
      await expect(
        adapter.deleteFile('file123', 'api-key'),
    ...
    it('throws ProviderNotFoundError on failure', async () => {
      ...
      await expect(adapter.deleteFile('file123', 'api-key')).rejects.toThrow(
        ProviderNotFoundError,
      );
    });
  });
